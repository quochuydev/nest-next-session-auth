  ...
  return (
    ...
    >
      ...
      <IndexTable.Cell>
        <TextField
          value={sku}
          onChange={(value) => handleChange(index, "sku", value)}
        />
      </IndexTable.Cell>
      <IndexTable.Cell>
        <TextField
          value={barcode}
          onChange={(value) => handleChange(index, "barcode", value)}
        />
      </IndexTable.Cell>
      <IndexTable.Cell>
        <TextField
          ...
          onChange={(value) =>
            handleChange(index, "options", [value, variant.options[1]])
          }
        />
      </IndexTable.Cell>
      <IndexTable.Cell>
        <TextField
          value={variant.options[1]}
          onChange={(value) =>
            handleChange(index, "options", [variant.options[0], value])
          }
        />
      </IndexTable.Cell>
      <IndexTable.Cell>
  ...
  const bulkActions = [
    {
      ...
      onAction: () =>
        console.log("Todo: implement bulk delete", selectedResources),
    },
  ];

  const handleChange = useCallback(
    (index, name, value) => {
      const newVariants = variants.map((e, i) =>
